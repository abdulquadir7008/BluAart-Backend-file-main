...
const start = async () => {
  ...
  //cron.schedule('*/2 * * * * ', async () => {
      ...
      try {
        ...
        console.log("TempCsvData", TempCsvData.length)
        ...
        for (const tempdata of TempCsvData) {
          ...
          console.log("dfdbulkartwork", FileUrl)
          const data = [];
          ...
          for (const elemnt of data) {
            ...
            try{
              if (MetaDataIpfs) {
                ...
                console.log("CollectionInfo.IPFSCid", CollectionInfo.IPFSCid)
            ...
            console.log("CollectionInfotest")
    ...
    cron.schedule('* * * * *', async () => {
      try {
        ...
        //console.log("responsematic.data", responsematic.data)
        const updateMaticQuery = `
        UPDATE "Exchange"
        SET "SGD" = $1, "GBP" = $2, "USD" = $3, "EUR" = $4
        WHERE "BaseCurrency" = 'MATIC'
        RETURNING *;
      `;

        const updatedMaticExchangeData = await pool.query(updateMaticQuery, [
          responsematic.data['matic-network'].sgd,
          responsematic.data['matic-network'].gbp,
          responsematic.data['matic-network'].usd,
          responsematic.data['matic-network'].eur,
        ]);

        //console.log('Updated MATIC Exchange Data:', updatedMaticExchangeData.rows);

        // Update data in the Exchange table for ETH
        const updateEthQuery = `
        UPDATE "Exchange"
        SET "SGD" = $1, "GBP" = $2, "USD" = $3, "EUR" = $4
        WHERE "BaseCurrency" = 'ETH'
        RETURNING *;
      `;

        const updatedEthExchangeData = await pool.query(updateEthQuery, [
          responseeth.data['ethereum'].sgd,
          responseeth.data['ethereum'].gbp,
          responseeth.data['ethereum'].usd,
          responseeth.data['ethereum'].eur,
        ]);
